...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/bridge-center/bridge/psychopy-bridge-tutorial/psychopy_bridge_tutorial/main_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    source_text = visual.TextStim(win=win, name='source_text',
        text='Did a male or female voice tell you this fact?\n\na. Male\nb. Female',
        font='Open Sans',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    for thisTrial_2 in trials_2:
        ...
        SourceTest = data.Routine(
            name='SourceTest',
            components=[source_text, source_response, feedback_text],
        )
        SourceTest.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        source_text.setColor('white', colorSpace='rgb')
        ...
        while continueRoutine and routineTimer.getTime() < 8.0:
            ...
            if source_response.keys:
                if (source_response.keys == str(correct_answer)) or (source_response.keys == correct_answer):
                    feedback_text.setColor('green')  # Correct answer
                    feedback_text.setText('Correct')  # Update feedback text
                else:
                    feedback_text.setColor('red')  # Incorrect answer
                    feedback_text.setText('Incorrect')  # Update feedback text
            else:
                feedback_text.setColor('red')  # Incorrect answer
                feedback_text.setText('Time out!')  # Update feedback text
            
            # *feedback_text* updates
            
            # if feedback_text is starting this frame...
            if feedback_text.status == NOT_STARTED and tThisFlip >= 3-frameTolerance:
                # keep track of start time/frame for later
